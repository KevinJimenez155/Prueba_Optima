...
<body>
            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('update', $user->id) }}" method="POST" class="form-group">
                        @method('PUT')
                        @csrf
                            <div class="container">
                                <div class="row">
                                    ...
                                    <div class="col-9">
                                        <div class="form-floating mb-3">
                                            <input type="text" class="form-control" id="name" name="name"  placeholder="Nombre Completo" value="{{old('name', $user->name)}}">
                                    ...
                                    <div class="col-3">
                                        <div class="form-floating mb-3">
                                            <input type="number" class="form-control" id="age" name="age" required placeholder="Edad" min="18" value="{{old('age', $user->age)}}">
                                    ...
                                    <div class="col-6">
                                        <div class="form-floating mb-3">
                                            <input type="text" class="form-control" id="phone" name="phone" required placeholder="Teléfono" pattern="[1-9]{1}[0-9]{9}" value="{{old('phone', $user->phone)}}">
                                    ...
                                    <div class="col-6">
                                        <div class="form-floating mb-3">
                                            <input type="email" class="form-control" id="email" name="email" required placeholder="Correo" value="{{old('email', $user->email )}}">
                                    ...
                                    <div class="col-12">
                                        <div class="form-floating mb-3">
                                            <select class="form-select"id="car_interest" name="car_interest" required aria-label="Floating label select example">
                                                ...
                                                @foreach ($cars as $car)
                                                    <option value="{{ $car->id }}" {{ old('car_interest', $user->car_model->car->id) == $car->id ? 'selected' : ''}}>{{ $car->name }}</option>
                                                @endforeach
                                            </select>
    ...
    <script>
        ...
        function getModels(){
            let id = $('#car_interest').val();
            if(id){
                let user_model_id = '{{$user->car_model_id}}'
                $('#myDiv').css("display", "block");
                $.ajax({
                        url:'{{ url('car') }}/'+id+'/models',
                        type: 'GET',
                        ...
                        for(let i = 0; i < data.length; i++){
                            $('#model_interest').append(
                                `<option value="${data[i].id}" ${user_model_id == data[i].id ? 'selected' : ''}>${data[i].name}</option>`
                            );
                        }
                    })
            }
